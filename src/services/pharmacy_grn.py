    ...
    def submit(self, data_in: PharmacyGrnWithSingleGrn, db: Session):
        ...
        if data_in.single_grn and len(data_in.single_grn) != 0:
            for i in data_in.single_grn:
                ...
                check_medicine_exist = pharmacy_total_current_stock_repo.get_by_two_key(
                    db=db, skip=0, limit=100, descending=False, count_results=True, medicine_id=i.medicine_id, pharmacy_id=data_in.grn.pharmacy_id)

                if check_medicine_exist[0].results != 0:
                    update_total = pharmacy_total_current_stock_repo.update(db=db, id=check_medicine_exist[1][0].id, data_update=PharmacyTotalCurrentStockUpdate(
                        quantity=check_medicine_exist[1][0].quantity + i.quantity
                    ))
