...
from api.v1.auth_dependcies import logged_in_admin_moderator_crm, logged_in_employee
...
def create_follow_up(data_in: FollowUpIn, service_id: int, current_user: Session = Depends(logged_in_admin_moderator_crm), db: Session = Depends(get_db)):
...
def update_followup(id: int, data_update: FollowUpUpdate, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_admin_moderator_crm)):
...
def search_by_service(id: int, skip: int = 0, limit: int = 15, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_admin_moderator_crm)):
...
def search_by_date(date: str, skip: int = 0, limit: int = 15, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_admin_moderator_crm)):
...
def search_by_status(follow_up_status: str, skip: int = 0, limit: int = 15, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_admin_moderator_crm)):
