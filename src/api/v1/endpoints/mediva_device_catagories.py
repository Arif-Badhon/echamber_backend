...
def all_catagory(db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
    data = mediva_device_catagory_service.get(db=db)
    return handle_result(data)


@router.post('/', response_model=MedivaDeviceCatagoryOut)
def create(data_in: MedivaDeviceCatagoryIn, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
    data = mediva_device_catagory_service.create(db=db, data_in=data_in)
    return handle_result(data)


@router.get('/{id}', response_model=MedivaDeviceCatagoryOut)
def single(id: int, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
    data = mediva_device_catagory_service.get_one(db=db, id=id)
    return handle_result(data)


@router.patch('/{id}', response_model=MedivaDeviceCatagoryOut)
def edit(id: int, data_update: MedivaDeviceCatagoryUpdate, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
    data = mediva_device_catagory_service.update(db=db, id=id, data_update=data_update)
    return handle_result(data)


@router.delete('/{id}')
def remove(id: int, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
