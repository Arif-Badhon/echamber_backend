...
def all_notice(skip: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
...
def create_notice(data_in: NoticeBase, db: Session = Depends(get_db), current_user: Session = Depends(logged_in_moderator)):
